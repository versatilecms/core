...
class MediaController extends Controller
{
    ...
    {
        $this->filesystem = config('versatile.storage.disk');
    }

    public function index()
    {
        // Check permission
        Versatile::canOrFail('browse_media');
        return Versatile::view('versatile::media.index');
    }
    ...
    public function new_folder(Request $request)
    {
        ...
        if (Storage::disk($this->filesystem)->exists($new_folder)) {
            $error = __('versatile::media.folder_exists_already');
        } elseif (Storage::disk($this->filesystem)->makeDirectory($new_folder)) {
            $success = true;
        } else {
            $error = __('versatile::media.error_creating_dir');
        }
    ...
    public function delete_file_folder(Request $request)
    {
        ...
        if ($type == 'folder') {
            if (!Storage::disk($this->filesystem)->deleteDirectory($fileFolder)) {
                $error = __('versatile::media.error_deleting_folder');
                $success = false;
            }
        } elseif (!Storage::disk($this->filesystem)->delete($fileFolder)) {
            $error = __('versatile::media.error_deleting_file');
            $success = false;
        }
    ...
    public function move_file(Request $request)
    {
        ...
        if (!file_exists($destination)) {
            if (Storage::disk($this->filesystem)->move($source, $destination)) {
                ...
                $error = __('versatile::media.error_moving');
            }
        } else {
            $error = __('versatile::media.error_already_exists');
        }
    ...
    public function rename_file(Request $request)
    {
        ...
        if (!Storage::disk($this->filesystem)->exists("{$location}/{$newFilename}")) {
            ...
            $error = __('versatile::media.error_may_exist');
        }
    ...
    {
        try {
            $realPath = Storage::disk($this->filesystem)
                ->getDriver()
                ->getAdapter()
                ->getPathPrefix();
            ...
            if (config('versatile.media.keep_filename')
            ...
            $success = true;
            $message = __('versatile::media.success_uploaded_file');
    ...
    private function getFiles($dir)
    {
        ...
        foreach ($storageItems as $item) {
            if ($item['type'] == 'dir') {
                ...
                if (empty(pathinfo($item['path'], PATHINFO_FILENAME)) && !config('versatile.hidden_files')) {
                    continue;
                }
                $files[] = [
                    'name'          => $item['basename'],
    ...
    public function remove(Request $request)
    {
        try {
            ...
            // Check permission
            Versatile::canOrFail('delete_'.$dataType->name);
            ...
            if (!isset($data->{$field})) {
                throw new Exception(__('versatile::generic.field_does_not_exist'), 400);
            }

            // Check if valid json
            if (is_null(@json_decode($data->{$field}))) {
                throw new Exception(__('versatile::json.invalid'), 500);
            }
            ...
            if (!array_key_exists($image, $fieldData)) {
                throw new Exception(__('versatile::media.image_does_not_exist'), 400);
            }
            ...
            return response()->json([
               'data' => [
                   'status'  => 200,
                   'message' => __('versatile::media.image_removed'),
               ],
            ]);
        } catch (Exception $e) {
            $code = 500;
            $message = __('versatile::generic.internal_error');
    ...
    public function crop(Request $request)
    {
        ...
        try {
            ...
            $success = true;
            $message = __('versatile::media.success_crop_image');
        } catch (Exception $e) {
            $success = false;
            $message = $e->getMessage();
        }
