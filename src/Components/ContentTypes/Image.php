    ...
    public function handle()
    {
        if ($this->request->hasFile($this->row->field)) {
            ...
            if ($this->isAnimatedGif($file)) {
                Storage::disk(config('versatile.storage.disk'))->put($fullPath, file_get_contents($file), 'public');
                $fullPathStatic = $path.$filename.'-static.'.$file->getClientOriginalExtension();
                Storage::disk(config('versatile.storage.disk'))->put($fullPathStatic, (string) $image, 'public');
            } else {
                Storage::disk(config('versatile.storage.disk'))->put($fullPath, (string) $image, 'public');
            }
            ...
            if (isset($this->options->thumbnails)) {
                foreach ($this->options->thumbnails as $thumbnails) {
                    ...
                    Storage::disk(config('versatile.storage.disk'))->put(
