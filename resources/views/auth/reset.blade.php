    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-5 col-md-4 login-sidebar">
            ...
            <div class="login-container">
                ...
                <p>{{ __('versatile::auth.create_new_password') }}</p>

                <form action="{{ route('versatile.password.reset.submit') }}" method="POST">
                    {{ csrf_field() }}
                    <input type="hidden" name="token" value="{{ $token }}">
                    <div class="form-group form-group-default" id="emailGroup">
                        <label>{{ __('versatile::generic.email') }}</label>
                        <div class="controls">
                            <input type="text" name="email" id="email" value="{{ $email }}" placeholder="{{ __('versatile::generic.email') }}" class="form-control" required>
                        </div>
                    </div>

                    <div class="form-group form-group-default" id="passwordGroup">
                        <label>{{ __('versatile::generic.password') }}</label>
                        <div class="controls">
                            <input type="password" name="password" placeholder="{{ __('versatile::generic.password') }}" class="form-control" required>
                        </div>
                    </div>

                    <div class="form-group form-group-default" id="passwordGroup">
                        <label>{{ __('versatile::generic.password_confirm') }}</label>
                        <div class="controls">
                            <input type="password" name="password_confirmation" placeholder="{{ __('versatile::generic.password_confirm') }}" class="form-control" required>
                        </div>
                    </div>

                    <button type="submit" class="btn btn-block login-button">
                        <span class="signingin hidden"><span class="versatile-refresh"></span> {{ __('versatile::generic.processing') }}...</span>
                        <span class="signin">{{ __('versatile::auth.reset_password') }}</span>
                    </button>
