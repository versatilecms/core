...
@section('page_title', __('versatile::generic.viewing').' '.__('versatile::generic.database'))

@section('page_header')
    <h1 class="page-title">
        <i class="versatile-data"></i> {{ __('versatile::generic.database') }}
        <a href="{{ route('versatile.database.create') }}" class="btn btn-success"><i class="versatile-plus"></i>
            {{ __('versatile::database.create_new_table') }}</a>
    </h1>
@stop
                ...
                <div class="panel panel-bordered">
                        <table class="table table-striped database-tables">
                            ...
                            <tr>
                                ...
                                <th style="text-align:right" colspan="2">{{ __('versatile::database.table_actions') }}</th>
                            </tr>
                            </thead>
                            ...
                            @foreach($tables as $table)
                                ...
                                <tr>
                                    ...
                                    <td class="actions">
                                        ...
                                        <a href="{{ route('versatile.database.edit', $table->name) }}"
                                        ...
                                        <a href="{{ route('versatile.database.show', $table->name) }}"
                                           ...
                                           class="btn btn-sm btn-warning pull-right desctable" style="display:inline; margin-right:10px;">
                                            <i class="versatile-eye"></i> {{ __('versatile::generic.view') }}
                                        </a>
                                    </td>
                                </tr>
                            @endforeach
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="versatile-trash"></i>  {!! __('versatile::bread.delete_bread_quest', ['table' => '<span id="delete_bread_name"></span>']) !!}</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('versatile.bread.delete', ['id' => null]) }}" id="delete_bread_form" method="POST">
                        {{ method_field('DELETE') }}
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('versatile::generic.close') }}"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="versatile-trash"></i> {!! __('versatile::database.delete_table_question', ['table' => '<span id="delete_table_name"></span>']) !!}</h4>
                </div>
                <div class="modal-footer">
                    <form action="#" id="delete_table_form" method="POST">
                        {{ method_field('DELETE') }}
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <input type="submit" class="btn btn-danger pull-right" value="{{ __('versatile::database.delete_table_confirm') }}">
                        <button type="button" class="btn btn-outline pull-right" style="margin-right:10px;"
                                data-dismiss="modal">{{ __('versatile::generic.cancel') }}
                        </button>
                    </form>
        ...
        $(function () {
            ...
            $('td.actions').on('click', '.delete_table', function (e) {
                ...
                if ($(this).hasClass('remove-bread-warning')) {
                    toastr.warning('{{ __('versatile::database.delete_bread_before_table') }}');
                } else {
                    $('#delete_table_name').text(table);

                    $('#delete_table_form')[0].action = '{{ route('versatile.database.destroy', ['database' => '__database']) }}'.replace('__database', table)
