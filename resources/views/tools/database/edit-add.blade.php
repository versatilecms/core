...
@if($db->action == 'update')
    @section('page_title', __('versatile::database.editing_table', ['table' => $db->table->name]))
@else
    @section('page_title', __('versatile::database.create_new_table'))
@endif
...
@section('page_header')
    <h1 class="page-title">
        ...
        @if($db->action == 'update')
            {{ __('versatile::database.editing_table', ['table' => $db->table->name]) }}
        @else
            {{ __('versatile::database.create_new_table') }}
        @endif
    </h1>
@stop
...
@section('javascript')
    @include('versatile::tools.database.vue-components.database-table-editor')
