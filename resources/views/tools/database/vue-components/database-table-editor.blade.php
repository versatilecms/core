...
<div class="panel panel-bordered">
    <div class="panel-body">
        ...
        @else
            <div class="col-md-6">
        @endif
                <label for="name">{{ __('versatile::database.table_name') }}</label><br>
                <input v-model.trim="table.name" type="text" class="form-control" placeholder="{{ __('versatile::database.table_name') }}" required pattern="{{ $db->identifierRegex }}">
            </div>

        @if($db->action == 'create')
            <div class="col-md-3 col-sm-4 col-xs-6">
                <label for="create_model">{{ __('versatile::database.create_model_table') }}</label><br>
                <input type="checkbox" name="create_model" data-toggle="toggle"
                       data-on="{{ __('versatile::generic.yes') }}" data-off="{{ __('versatile::generic.no') }}">
            </div>
             <div class="col-md-3 col-sm-4 col-xs-6">
                <label for="create_migration">{{ __('versatile::database.create_migration') }}</label><br>
        ...
        <div v-if="compositeIndexes.length" v-once class="alert alert-danger">
            <p>{{ __('versatile::database.no_composites_warning') }}</p>
        </div>

        <div id="alertsContainer"></div>

        <template v-if="tableHasColumns">
            <p>{{ __('versatile::database.table_columns') }}</p>

            <table class="table table-bordered" style="width:100%;">
                <thead>
                <tr>
                    <th>{{ __('versatile::generic.name') }}</th>
                    <th>{{ __('versatile::generic.type') }}</th>
                    <th>{{ __('versatile::generic.length') }}</th>
                    <th>{{ __('versatile::generic.not_null') }}</th>
                    <th>{{ __('versatile::generic.unsigned') }}</th>
                    <th>{{ __('versatile::generic.auto_increment') }}</th>
                    <th>{{ __('versatile::generic.index') }}</th>
                    <th>{{ __('versatile::generic.default') }}</th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
        ...
        <div v-else>
          <p>{{ __('versatile::database.table_no_columns') }}</p>
        </div>
    ...
    <div class="panel-footer">
        <input type="submit" class="btn btn-primary pull-right"
               value="@if($db->action == 'update'){{ __('versatile::database.update_table') }}@else{{ __('versatile::database.create_new_table') }}@endif"
...
@include('versatile::tools.database.vue-components.database-column')
@include('versatile::tools.database.vue-components.database-table-helper-buttons')
        ...
        methods: {
            addColumn(column) {
                ...
                if (column.name && this.hasColumn(column.name)) {
                    return toastr.error("{{ __('versatile::database.column') }} " + column.name + " {{ __('versatile::database.already_exists') }}");
                }
            ...
            renameColumn(column) {
                ...
                if ((existingColumn = this.getColumn(newName)) && (existingColumn !== column)) {
                    return toastr.error("{{ __('versatile::database.column') }} " + newName + " {{ __('versatile::database.already_exists') }}");
                }
            ...
            addIndex(index) {
                if (index.type == 'PRIMARY') {
                    if (this.table.primaryKeyName) {
                        return toastr.error("{{ __('versatile::database.table_has_index') }}");
                    }
