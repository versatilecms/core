...
<tr class="newTableRow">
    ...
    <td>
        <select :value="index.type" @change="onIndexTypeChange"
                ...
                class="form-control">
            <option value=""></option>
            <option value="INDEX">{{ __('versatile::database.index') }}</option>
            <option value="UNIQUE">{{ __('versatile::database.unique') }}</option>
            <option value="PRIMARY">{{ __('versatile::database.primary') }}</option>
        </select>
        <small v-if="column.composite" v-once>{{ __('versatile::database.composite_warning') }}</small>
    </td>
    ...
    <td>
        <div class="btn btn-danger delete-row" @click="deleteColumn"><i class="versatile-trash"></i></div>
    </td>
...
@include('versatile::tools.database.vue-components.database-types')
@include('versatile::tools.database.vue-components.database-column-default')
    ...
    Vue.component('database-column', {
        ...
        methods: {
            ...
            onIndexTypeChange(event) {
                if (this.column.name == '') {
                    return toastr.error("{{ __('versatile::database.name_warning') }}");
                }
